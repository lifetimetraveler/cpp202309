...
int main() {
	...
	int map[mapY][mapX];//맵을 위한 배열 선언

	//map 배열에 지도를 입력(파일을 이용해서)
	ifstream for_map("map.txt");//지도 파일을 받아옴
	try//예외 처리. 파일이 없거나 파일의 문자가 지도보다 많으면 에러메시지 출력.
	{
		if (!for_map)//지도파일 못 가져오면 에러메시지 출력
			throw invalid_argument("파일을 불러오지 못했습니다.");

		int i = 0;//맵 파일에서 문자를 가져온 횟수를 카운트하기 위함
		char n;//맵 파일의 문자를 받을 변수
		int map_pos;//정수화한 맵 파일의 문자를 위한 변수
		while (1)
		{
			for_map.get(n);//문자를 가져옴
			map_pos = n - '0';//문자를 정수화함


			if (for_map.eof())//파일 끝이면 맵 배열에 입력을 멈추기위함. 0이 안들어가도록
			{
				break;
			}
			if (mapX * mapY <= i)//반복 횟수(맵파일에서 받아온 횟수)가 map 배열을 넘어서면 에러메시지출력
				throw invalid_argument("파일의 크기가 맵을 벗어났습니다.");
			map[i / 5][i % 5] = map_pos;//정수화한 맵 파일의 문자를 배열에 순차적으로 할당.
			i++;//문자 받아온 횟수 카운트
		}
	}
	catch(exception& error)//예외처리 에러메시지 출력을 위함.
	{
		cout << "에러 : " << error.what() << endl;
	}
	

	 //= { {0, 1, 2, 0, 4},//배열에 여러 오브젝트들을 할당
		//			{1, 0, 0, 2, 0},
		//			{0, 0, 0, 0, 0},
		//			{0, 2, 3, 0, 0},
		//			{3, 0, 0, 0, 2} };
